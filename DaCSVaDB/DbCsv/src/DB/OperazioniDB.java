...
public class OperazioniDB {
	...
	public static final String[] tableName = {"Circuits", "Constructors", "Drivers", "Races", "Results"};
	public static final String[] fileName = {"circuits.csv", "constructors.csv", "driver.csv", "races.csv", "results.csv"};
            ...
            if (valori[i].equals("\\N")) {
                ...
                if (valori[i].charAt(0) == '"') {
                    for (int j = 0; j < valori[i].length(); j++) {
                        if (valori[i].charAt(j) == '\'') {
                            valori[i] = valori[i].substring(0, j) + valori[i].substring(j+1);
                        }
                    }
                    strCampo = valori[i].substring(1, valori[i].length() - 1);
                    strQuery += "'" + strCampo + "',";
